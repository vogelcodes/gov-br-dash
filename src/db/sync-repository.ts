...
export class SqliteSyncRepository {
  ...
  upsertArpItem(numeroControlePncpAta: string, item: ArpItem): void {
    ...
    this.db.prepare(`
      ...
      ON CONFLICT(numero_controle_pncp_ata, numero_item) DO UPDATE SET
        ...
        raw_json = excluded.raw_json,
        last_synced_at = excluded.last_synced_at
    `).run(numeroControlePncpAta, item.numeroItem, normalizeDigits(item.niFornecedor), JSON.stringify(item), now);
  }
  ...
  upsertEmpenho(id: string, numeroControlePncpAta: string, numeroItem: string, raw: unknown): void {
    const now = new Date().toISOString();
    this.db.prepare(`
  ...
  findArp(numeroControlePncpAta: string): StoredArpRecord | null {
    const row = this.db.prepare("SELECT codigo_uasg, raw_json, last_synced_at FROM arps WHERE numero_controle_pncp_ata = ?").get(numeroControlePncpAta) as { codigo_uasg: string; raw_json: string; last_synced_at: string } | undefined;
    return row ? { codigoUasg: row.codigo_uasg, raw: JSON.parse(row.raw_json) as Arp, lastSyncedAt: row.last_synced_at } : null;
  }

  findArpForUasg(numeroControlePncpAta: string, codigoUasg: string): StoredJsonRecord<Arp> | null {
    const row = this.db.prepare("SELECT raw_json, last_synced_at FROM arps WHERE numero_controle_pncp_ata = ? AND codigo_uasg = ?").get(numeroControlePncpAta, codigoUasg) as { raw_json: string; last_synced_at: string } | undefined;
    return row ? { raw: JSON.parse(row.raw_json) as Arp, lastSyncedAt: row.last_synced_at } : null;
  }

  findItem(numeroControlePncpAta: string, numeroItem: string): StoredJsonRecord<ArpItem> | null {
    const row = this.db.prepare("SELECT raw_json, last_synced_at FROM arp_items WHERE numero_controle_pncp_ata = ? AND numero_item = ?").get(numeroControlePncpAta, numeroItem) as { raw_json: string; last_synced_at: string } | undefined;
    return row ? { raw: JSON.parse(row.raw_json) as ArpItem, lastSyncedAt: row.last_synced_at } : null;
  }

  findPessoaJuridica(cnpj: string): StoredJsonRecord | null {
    const row = this.db.prepare("SELECT raw_json, last_synced_at FROM pessoas_juridicas WHERE cnpj = ?").get(normalizeDigits(cnpj)) as { raw_json: string; last_synced_at: string } | undefined;
